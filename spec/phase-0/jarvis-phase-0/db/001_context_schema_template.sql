...
CREATE TABLE {{SCHEMA}}.event (
    ...
    CONSTRAINT event_context_chk CHECK (context_id = '{{CONTEXT_ID}}')
);
CREATE UNIQUE INDEX event_idempotency_uq ON {{SCHEMA}}.event (idempotency_key);
CREATE INDEX event_type_time_idx ON {{SCHEMA}}.event (event_type, event_time DESC);
...
CREATE TABLE {{SCHEMA}}.task (
    ...
    CONSTRAINT task_context_chk CHECK (context_id = '{{CONTEXT_ID}}')
);
CREATE UNIQUE INDEX task_idempotency_uq ON {{SCHEMA}}.task (idempotency_key);
CREATE INDEX task_open_due_idx ON {{SCHEMA}}.task (status, due_at);

-- ---------------------------------------------------------------------
-- Aktionen - zentrale Idempotenzsicherung
-- ---------------------------------------------------------------------
CREATE TABLE {{SCHEMA}}.action (
    ...
    CONSTRAINT action_context_chk CHECK (context_id = '{{CONTEXT_ID}}'),
...
CREATE UNIQUE INDEX action_idempotency_uq ON {{SCHEMA}}.action (idempotency_key);
CREATE INDEX action_status_idx ON {{SCHEMA}}.action (status, next_attempt_at);

-- ---------------------------------------------------------------------
-- Ausfuehrungssperre gegen parallele Doppelausfuehrung
-- ---------------------------------------------------------------------
CREATE TABLE {{SCHEMA}}.action_lock (
    idempotency_key    char(64) PRIMARY KEY,
    action_id          text NOT NULL REFERENCES {{SCHEMA}}.action(action_id),
...
CREATE TABLE {{SCHEMA}}.approval (
    ...
    body               jsonb NOT NULL,
    CONSTRAINT approval_context_chk CHECK (context_id = '{{CONTEXT_ID}}')
);
CREATE UNIQUE INDEX approval_open_uq
    ON {{SCHEMA}}.approval (action_id, action_fingerprint)
    WHERE status = 'pending';
CREATE UNIQUE INDEX approval_token_uq ON {{SCHEMA}}.approval (token_hash);
...
CREATE TABLE {{SCHEMA}}.evidence (
    ...
    body                jsonb NOT NULL,
    created_at          timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT evidence_context_chk CHECK (context_id = '{{CONTEXT_ID}}'),
...
CREATE INDEX evidence_action_idx ON {{SCHEMA}}.evidence (action_id);

-- ---------------------------------------------------------------------
-- Fehler und Eskalationen
-- ---------------------------------------------------------------------
CREATE TABLE {{SCHEMA}}.error_event (
    ...
    body               jsonb NOT NULL,
    CONSTRAINT error_context_chk CHECK (context_id = '{{CONTEXT_ID}}')
);
...
CREATE TABLE {{SCHEMA}}.action_log (
    ...
    body               jsonb NOT NULL,
    corrects_log_id    bigint REFERENCES {{SCHEMA}}.action_log(log_id),
    CONSTRAINT log_context_chk CHECK (context_id = '{{CONTEXT_ID}}')
);
...
CREATE OR REPLACE FUNCTION {{SCHEMA}}.deny_log_mutation()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER action_log_deny_update
    BEFORE UPDATE ON {{SCHEMA}}.action_log
    FOR EACH ROW EXECUTE FUNCTION {{SCHEMA}}.deny_log_mutation();

CREATE TRIGGER action_log_deny_delete
    BEFORE DELETE ON {{SCHEMA}}.action_log
    FOR EACH ROW EXECUTE FUNCTION {{SCHEMA}}.deny_log_mutation();

CREATE TRIGGER action_log_deny_truncate
    BEFORE TRUNCATE ON {{SCHEMA}}.action_log
    FOR EACH STATEMENT EXECUTE FUNCTION {{SCHEMA}}.deny_log_mutation();

-- ---------------------------------------------------------------------
-- Dokumentregister und Dublettenerkennung
-- ---------------------------------------------------------------------
CREATE TABLE {{SCHEMA}}.document_index (
...
CREATE UNIQUE INDEX document_content_hash_uq ON {{SCHEMA}}.document_index (content_hash);

-- ---------------------------------------------------------------------
-- Gedaechtnis - Schema in Phase 0, produktive Nutzung ab Phase 4
-- ---------------------------------------------------------------------
CREATE TABLE {{SCHEMA}}.memory_entry (
    ...
    superseded_by      text REFERENCES {{SCHEMA}}.memory_entry(memory_id),
    expires_at         timestamptz,
    body               jsonb NOT NULL,
    created_at         timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT memory_context_chk CHECK (context_id = '{{CONTEXT_ID}}')
);
CREATE INDEX memory_store_status_idx ON {{SCHEMA}}.memory_entry (memory_store, status);
...
GRANT SELECT, INSERT, UPDATE ON
    ...
    {{SCHEMA}}.evidence,
    {{SCHEMA}}.error_event,
    {{SCHEMA}}.document_index,
    {{SCHEMA}}.memory_entry
TO {{DB_USER}};

-- Abgelaufene Sperren duerfen nach Statusabgleich entfernt werden
GRANT DELETE ON {{SCHEMA}}.action_lock TO {{DB_USER}};

-- Fachprotokoll: ausschliesslich lesen und einfuegen
GRANT SELECT, INSERT ON {{SCHEMA}}.action_log TO {{DB_USER}};
REVOKE UPDATE, DELETE, TRUNCATE ON {{SCHEMA}}.action_log FROM {{DB_USER}};
