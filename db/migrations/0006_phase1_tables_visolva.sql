-- =====================================================================
-- JARVIS Phase 1.0 - Migration 0006 - Phase-1-Tabellen in jarvis_visolva
--
-- Erzeugt aus: db/templates/004_phase1_context_tables.template.sql via tools/render_phase1_tables.py --context arbeitgeber_visolva
...
-- context_id = 'arbeitgeber_visolva' und erhaelt die Sequenzrechte.
--
...
-- ---------------------------------------------------------------------
DROP TABLE IF EXISTS jarvis_visolva.document_index;

CREATE TABLE jarvis_visolva.document (
    ...
    duplicate_of          text REFERENCES jarvis_visolva.document(document_id),
...
CREATE UNIQUE INDEX document_content_hash_uq ON jarvis_visolva.document (content_hash);
CREATE INDEX document_status_idx           ON jarvis_visolva.document (status, received_at DESC);
CREATE INDEX document_case_idx             ON jarvis_visolva.document (case_id);
CREATE INDEX document_text_fingerprint_idx ON jarvis_visolva.document (text_fingerprint);
CREATE INDEX document_source_external_idx  ON jarvis_visolva.document (source_external_id);
...
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_visolva."case" (
    ...
    created_at        timestamptz NOT NULL DEFAULT now(),
    updated_at        timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT case_context_chk CHECK (context_id = 'arbeitgeber_visolva'),
...
CREATE UNIQUE INDEX case_number_uq ON jarvis_visolva."case" (case_number);
CREATE INDEX case_status_deadline_idx ON jarvis_visolva."case" (status, next_deadline_at);

-- Der Vorgangsbezug des Dokuments wird erst jetzt verknuepfbar, weil beide
-- Tabellen vorhanden sein muessen.
ALTER TABLE jarvis_visolva.document
    ADD CONSTRAINT document_case_fk
    FOREIGN KEY (case_id) REFERENCES jarvis_visolva."case"(case_id);
...
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_visolva.case_identifier (
    case_identifier_id     bigserial PRIMARY KEY,
    context_id             text NOT NULL DEFAULT 'arbeitgeber_visolva',
    case_id                text NOT NULL REFERENCES jarvis_visolva."case"(case_id),
    ...
    first_seen_document_id text REFERENCES jarvis_visolva.document(document_id),
    created_at             timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT case_identifier_context_chk CHECK (context_id = 'arbeitgeber_visolva')
);
CREATE UNIQUE INDEX case_identifier_value_uq
    ON jarvis_visolva.case_identifier (identifier_type, value_normalized);
CREATE INDEX case_identifier_case_idx ON jarvis_visolva.case_identifier (case_id);
...
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_visolva.document_text (
    document_id          text PRIMARY KEY REFERENCES jarvis_visolva.document(document_id),
    context_id           text NOT NULL DEFAULT 'arbeitgeber_visolva',
    content_hash         text NOT NULL
                         CHECK (content_hash ~ '^sha256:[a-f0-9]{64}$'),
    ...
    CONSTRAINT document_text_context_chk CHECK (context_id = 'arbeitgeber_visolva')
);
...
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_visolva.document_extraction (
    extraction_id                  bigserial PRIMARY KEY,
    context_id                     text NOT NULL DEFAULT 'arbeitgeber_visolva',
    document_id                    text NOT NULL REFERENCES jarvis_visolva.document(document_id),
    schema_version                 text NOT NULL,
    ...
    body                           jsonb NOT NULL,
    created_at                     timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT document_extraction_context_chk CHECK (context_id = 'arbeitgeber_visolva')
);
CREATE INDEX document_extraction_doc_idx
    ON jarvis_visolva.document_extraction (document_id, extracted_at DESC);

-- ---------------------------------------------------------------------
-- Analysen nach document_analysis.schema.json
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_visolva.document_analysis (
    ...
    body               jsonb NOT NULL,
    created_at         timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT document_analysis_context_chk CHECK (context_id = 'arbeitgeber_visolva')
);
CREATE INDEX document_analysis_doc_idx
    ON jarvis_visolva.document_analysis (document_id, analyzed_at DESC);
...
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_visolva.case_number_seq (
    context_id   text NOT NULL DEFAULT 'arbeitgeber_visolva',
    ...
    CONSTRAINT case_number_seq_context_chk CHECK (context_id = 'arbeitgeber_visolva')
);

CREATE OR REPLACE FUNCTION jarvis_visolva.next_case_number(p_year integer)
RETURNS text
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    VALUES ('arbeitgeber_visolva', p_year, 1, now())
    ON CONFLICT (context_id, year) DO UPDATE
        SET last_number = jarvis_visolva.case_number_seq.last_number + 1,
            updated_at  = now()
...
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_visolva.test_approval_record (
    ...
    CONSTRAINT test_approval_record_context_chk CHECK (context_id = 'arbeitgeber_visolva')
);
CREATE INDEX test_approval_record_action_idx
    ON jarvis_visolva.test_approval_record (action_id);
...
GRANT SELECT, INSERT, UPDATE ON
    jarvis_visolva.document,
    jarvis_visolva."case",
    jarvis_visolva.case_identifier,
    jarvis_visolva.document_text,
    jarvis_visolva.document_extraction,
    jarvis_visolva.document_analysis,
    jarvis_visolva.case_number_seq,
    jarvis_visolva.test_approval_record
TO jv_visolva_user;
...
-- Nummernvergabe
GRANT EXECUTE ON FUNCTION jarvis_visolva.next_case_number(integer) TO jv_visolva_user;
