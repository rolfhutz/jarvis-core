-- =====================================================================
-- JARVIS Phase 1.0 - Migration 0003 - Kontextschema jarvis_privat
--
-- Erzeugt aus: spec/phase-0/jarvis-phase-0/db/001_context_schema_template.sql via tools/render_context_schema.py --context privat
...
--
-- Platzhalter:
--   jarvis_privat      Zielschema, z. B. jarvis_privat
--   privat  Kontextkennung, z. B. privat
--   jv_privat_user     Datenbankbenutzer des Kontexts, z. B. jv_privat_user
--
...
CREATE SCHEMA IF NOT EXISTS jarvis_privat;

-- ---------------------------------------------------------------------
-- Ereignisse
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.event (
    event_id           text PRIMARY KEY,
    schema_version     text NOT NULL,
    event_type         text NOT NULL,
    event_time         timestamptz NOT NULL,
    received_at        timestamptz NOT NULL,
    context_id         text NOT NULL DEFAULT 'privat',
    ...
    CONSTRAINT event_context_chk CHECK (context_id = 'privat')
);
CREATE UNIQUE INDEX event_idempotency_uq ON jarvis_privat.event (idempotency_key);
CREATE INDEX event_type_time_idx ON jarvis_privat.event (event_type, event_time DESC);

-- ---------------------------------------------------------------------
-- Aufgaben
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.task (
    ...
    CONSTRAINT task_context_chk CHECK (context_id = 'privat')
);
CREATE UNIQUE INDEX task_idempotency_uq ON jarvis_privat.task (idempotency_key);
CREATE INDEX task_open_due_idx ON jarvis_privat.task (status, due_at);

-- ---------------------------------------------------------------------
-- Aktionen - zentrale Idempotenzsicherung
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.action (
    ...
    CONSTRAINT action_context_chk CHECK (context_id = 'privat'),
...
CREATE UNIQUE INDEX action_idempotency_uq ON jarvis_privat.action (idempotency_key);
CREATE INDEX action_status_idx ON jarvis_privat.action (status, next_attempt_at);

-- ---------------------------------------------------------------------
-- Ausfuehrungssperre gegen parallele Doppelausfuehrung
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.action_lock (
    idempotency_key    char(64) PRIMARY KEY,
    action_id          text NOT NULL REFERENCES jarvis_privat.action(action_id),
...
-- Freigaben
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.approval (
    ...
    body               jsonb NOT NULL,
    CONSTRAINT approval_context_chk CHECK (context_id = 'privat')
);
CREATE UNIQUE INDEX approval_open_uq
    ON jarvis_privat.approval (action_id, action_fingerprint)
    WHERE status = 'pending';
CREATE UNIQUE INDEX approval_token_uq ON jarvis_privat.approval (token_hash);

-- ---------------------------------------------------------------------
-- Ergebnisnachweise
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.evidence (
    ...
    body                jsonb NOT NULL,
    created_at          timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT evidence_context_chk CHECK (context_id = 'privat'),
...
CREATE INDEX evidence_action_idx ON jarvis_privat.evidence (action_id);

-- ---------------------------------------------------------------------
-- Fehler und Eskalationen
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.error_event (
    ...
    body               jsonb NOT NULL,
    CONSTRAINT error_context_chk CHECK (context_id = 'privat')
);

-- ---------------------------------------------------------------------
-- Fachliches Aktionsprotokoll - append-only
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.action_log (
    ...
    body               jsonb NOT NULL,
    corrects_log_id    bigint REFERENCES jarvis_privat.action_log(log_id),
    CONSTRAINT log_context_chk CHECK (context_id = 'privat')
);
...
CREATE OR REPLACE FUNCTION jarvis_privat.deny_log_mutation()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER action_log_deny_update
    BEFORE UPDATE ON jarvis_privat.action_log
    FOR EACH ROW EXECUTE FUNCTION jarvis_privat.deny_log_mutation();

CREATE TRIGGER action_log_deny_delete
    BEFORE DELETE ON jarvis_privat.action_log
    FOR EACH ROW EXECUTE FUNCTION jarvis_privat.deny_log_mutation();

CREATE TRIGGER action_log_deny_truncate
    BEFORE TRUNCATE ON jarvis_privat.action_log
    FOR EACH STATEMENT EXECUTE FUNCTION jarvis_privat.deny_log_mutation();

-- ---------------------------------------------------------------------
-- Dokumentregister und Dublettenerkennung
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.document_index (
    ...
    body                jsonb NOT NULL,
    CONSTRAINT document_context_chk CHECK (context_id = 'privat')
);
CREATE UNIQUE INDEX document_content_hash_uq ON jarvis_privat.document_index (content_hash);

-- ---------------------------------------------------------------------
-- Gedaechtnis - Schema in Phase 0, produktive Nutzung ab Phase 4
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.memory_entry (
    ...
    superseded_by      text REFERENCES jarvis_privat.memory_entry(memory_id),
    expires_at         timestamptz,
    body               jsonb NOT NULL,
    created_at         timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT memory_context_chk CHECK (context_id = 'privat')
);
CREATE INDEX memory_store_status_idx ON jarvis_privat.memory_entry (memory_store, status);
...
GRANT USAGE ON SCHEMA jarvis_privat TO jv_privat_user;

-- Arbeitstabellen: lesen, schreiben, aendern
GRANT SELECT, INSERT, UPDATE ON
    jarvis_privat.event,
    jarvis_privat.task,
    jarvis_privat.action,
    jarvis_privat.action_lock,
    jarvis_privat.approval,
    jarvis_privat.evidence,
    jarvis_privat.error_event,
    jarvis_privat.document_index,
    jarvis_privat.memory_entry
TO jv_privat_user;

-- Abgelaufene Sperren duerfen nach Statusabgleich entfernt werden
GRANT DELETE ON jarvis_privat.action_lock TO jv_privat_user;

-- Fachprotokoll: ausschliesslich lesen und einfuegen
GRANT SELECT, INSERT ON jarvis_privat.action_log TO jv_privat_user;
REVOKE UPDATE, DELETE, TRUNCATE ON jarvis_privat.action_log FROM jv_privat_user;
...
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA jarvis_privat TO jv_privat_user;
ALTER DEFAULT PRIVILEGES IN SCHEMA jarvis_privat
    GRANT USAGE, SELECT ON SEQUENCES TO jv_privat_user;
