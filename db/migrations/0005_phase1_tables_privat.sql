-- =====================================================================
-- JARVIS Phase 1.0 - Migration 0005 - Phase-1-Tabellen in jarvis_privat
--
-- Erzeugt aus: db/templates/004_phase1_context_tables.template.sql via tools/render_phase1_tables.py --context privat
...
-- context_id = 'privat' und erhaelt die Sequenzrechte.
--
...
-- ---------------------------------------------------------------------
DROP TABLE IF EXISTS jarvis_privat.document_index;

CREATE TABLE jarvis_privat.document (
    ...
    duplicate_of          text REFERENCES jarvis_privat.document(document_id),
...
CREATE UNIQUE INDEX document_content_hash_uq ON jarvis_privat.document (content_hash);
CREATE INDEX document_status_idx           ON jarvis_privat.document (status, received_at DESC);
CREATE INDEX document_case_idx             ON jarvis_privat.document (case_id);
CREATE INDEX document_text_fingerprint_idx ON jarvis_privat.document (text_fingerprint);
CREATE INDEX document_source_external_idx  ON jarvis_privat.document (source_external_id);
...
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat."case" (
    ...
    created_at        timestamptz NOT NULL DEFAULT now(),
    updated_at        timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT case_context_chk CHECK (context_id = 'privat'),
...
CREATE UNIQUE INDEX case_number_uq ON jarvis_privat."case" (case_number);
CREATE INDEX case_status_deadline_idx ON jarvis_privat."case" (status, next_deadline_at);

-- Der Vorgangsbezug des Dokuments wird erst jetzt verknuepfbar, weil beide
-- Tabellen vorhanden sein muessen.
ALTER TABLE jarvis_privat.document
    ADD CONSTRAINT document_case_fk
    FOREIGN KEY (case_id) REFERENCES jarvis_privat."case"(case_id);
...
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.case_identifier (
    case_identifier_id     bigserial PRIMARY KEY,
    context_id             text NOT NULL DEFAULT 'privat',
    case_id                text NOT NULL REFERENCES jarvis_privat."case"(case_id),
    ...
    first_seen_document_id text REFERENCES jarvis_privat.document(document_id),
    created_at             timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT case_identifier_context_chk CHECK (context_id = 'privat')
);
CREATE UNIQUE INDEX case_identifier_value_uq
    ON jarvis_privat.case_identifier (identifier_type, value_normalized);
CREATE INDEX case_identifier_case_idx ON jarvis_privat.case_identifier (case_id);
...
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.document_text (
    document_id          text PRIMARY KEY REFERENCES jarvis_privat.document(document_id),
    context_id           text NOT NULL DEFAULT 'privat',
    content_hash         text NOT NULL
                         CHECK (content_hash ~ '^sha256:[a-f0-9]{64}$'),
    ...
    CONSTRAINT document_text_context_chk CHECK (context_id = 'privat')
);
...
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.document_extraction (
    extraction_id                  bigserial PRIMARY KEY,
    context_id                     text NOT NULL DEFAULT 'privat',
    document_id                    text NOT NULL REFERENCES jarvis_privat.document(document_id),
    schema_version                 text NOT NULL,
    ...
    body                           jsonb NOT NULL,
    created_at                     timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT document_extraction_context_chk CHECK (context_id = 'privat')
);
CREATE INDEX document_extraction_doc_idx
    ON jarvis_privat.document_extraction (document_id, extracted_at DESC);

-- ---------------------------------------------------------------------
-- Analysen nach document_analysis.schema.json
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.document_analysis (
    ...
    body               jsonb NOT NULL,
    created_at         timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT document_analysis_context_chk CHECK (context_id = 'privat')
);
CREATE INDEX document_analysis_doc_idx
    ON jarvis_privat.document_analysis (document_id, analyzed_at DESC);
...
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.case_number_seq (
    context_id   text NOT NULL DEFAULT 'privat',
    ...
    CONSTRAINT case_number_seq_context_chk CHECK (context_id = 'privat')
);

CREATE OR REPLACE FUNCTION jarvis_privat.next_case_number(p_year integer)
RETURNS text
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    VALUES ('privat', p_year, 1, now())
    ON CONFLICT (context_id, year) DO UPDATE
        SET last_number = jarvis_privat.case_number_seq.last_number + 1,
            updated_at  = now()
...
-- ---------------------------------------------------------------------
CREATE TABLE jarvis_privat.test_approval_record (
    ...
    CONSTRAINT test_approval_record_context_chk CHECK (context_id = 'privat')
);
CREATE INDEX test_approval_record_action_idx
    ON jarvis_privat.test_approval_record (action_id);
...
GRANT SELECT, INSERT, UPDATE ON
    jarvis_privat.document,
    jarvis_privat."case",
    jarvis_privat.case_identifier,
    jarvis_privat.document_text,
    jarvis_privat.document_extraction,
    jarvis_privat.document_analysis,
    jarvis_privat.case_number_seq,
    jarvis_privat.test_approval_record
TO jv_privat_user;
...
-- Nummernvergabe
GRANT EXECUTE ON FUNCTION jarvis_privat.next_case_number(integer) TO jv_privat_user;
